...
| 熔炉桌宠 | `FurnaceSprite` | 贴右、距底约三分之一；两帧慢循环 GIF（偶尔招手 / 捧书 / 交叠）+ 戳一下；收起后仍是整个人 |
...
| 中栏输入 | `XSender` | **`ref` 取文** + `@submit`；`@paste-file` 粘贴上传；`submit-type="enter"`；工具栏 `/` `@` `#` · 附件 · **复制**；Enter/闸门说明在工具栏星标悬停，不再铺折行灰字 |
