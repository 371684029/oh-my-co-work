...
| 前端纯逻辑 | `web/test/*.test.mjs`（5 文件） | 同上（浏览器不用开） | **26 例** | `npm run test:web` |
| 自愈/冒烟脚本 | `scripts/selftest-*.mjs` + `test-enter-send.mjs` | 独立脚本，**不走 test runner** | 6 + 1 个 | `node scripts/selftest-xxx.mjs` |
...
`scripts/selftest-forward-jump.mjs`、`selftest-session-ops.mjs`、`selftest-v1-priority.mjs`、`selftest-v1-stability.mjs`、`selftest-furnace-scroll.mjs`、`selftest-docs-rail-toolbar.mjs`——**不走 test runner**。前四个是独立长流程脚本（起真实服务/跑完整流）；`selftest-furnace-scroll.mjs` 测熔炉 GUI 对话滚动与 TUI 右侧历史抽屉；`selftest-docs-rail-toolbar.mjs` 测第三栏文档中心工具栏窄宽换行。另有 `test-enter-send.mjs`（输入框回车发送的单点验证）。与单测的分工：单测管"单元语义不变"，selftest 管"整条流程能走通"。**注意**：长流程脚本默认针对真实运行中的服务与真实数据（如 `selftest-session-ops` 断言库里已有群模板），不要在含重要数据的实例上随手跑。
