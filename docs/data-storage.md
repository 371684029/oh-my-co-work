...
| **从节点重新开始** | 归档前/后均可：`POST /sessions/:id/restart-from-node`。**统一**按模板自该步起（跳过场外）**线性追加克隆**并开跑，历史保留。场外无此操作。已归档会话会先解档。 |
| **归档 / 解档** | 归档 = 杀进程释放资源；`POST /sessions/:id/unarchive` 或发送消息可解档。**不**再因发送自动开空白新任务（空白任务请另开聊）。 |
