...
**成员 script** 与 **快捷指令 shell（含脚本锚点）** 共用 `scriptWorkDir`（兼容旧字段 `scriptDir`）与 `resolveScriptFilePath`。**与会话 / 群 / 成员「工作文件夹」无关。**

| 场景 | 说明 |
|------|------|
| 成员 · 脚本文件 / 命令 | 浏览选脚本或锚点脚本 → 自动 `scriptWorkDir`；命令含 `node index.mjs` 等时保存/运行也会推断 |
| 快捷指令 · 跑脚本 | 填 `scriptWorkDir` / 选脚本 / **继承成员脚本工作目录**；**不会**用会话工作文件夹顶替脚本目录 |
| 快捷指令 · 开编辑器/资源管理器 | `{folder}` / spawn cwd 为「工作目录目标」（会话/群文件夹或自定义路径） |

保存成员或快捷指令时会根据脚本路径补全 `scriptWorkDir`（并镜像 `scriptDir` 以兼容旧配置）。
