...
| 属性 | 内容 |
|------|------|
| 目标版本 | `4.x`（首版 `4.0.0`，当前 `4.2.0`） |
| 状态 | **4.2.0 源码已对齐**（4.0 文档中心 + 4.1 检索导出 + 4.2 更新检查/备份恢复；运行包 zip 仍标 4.0.0，待重打） |
| 更新日期 | 2026-09-02 |
...
- [x] 全文搜索：输入关键词 → 内存扫描 `journals` 命中文件 + 行摘要
- [x] 节点文档徽标：状态 / 适配 / 克隆角标沿用流程轨语义
- [x] 「打包本群全部 MD」：服务端打 zip 下载（复用 adaptBackup 的打包基建）
- [ ] 代码高亮评估（按需引入，不默认全量）
...
- [x] 同步隐私文案：about.json 的 extraNotes「不会自动联网检查」改为「启动时只读取远端最新版本号与更新日志，不上传任何本机数据，可在设置关闭」；关于页同步
