...
主题：**把散落的协同文档聚起来**。会话跑完留下群报告 `ANNOUNCEMENT.md`、会话索引 `README.md`、节点台账 `step-*.md`——它们都在 `data/journals/sessions/{id}/` 下，但用户要靠文件管理器翻目录才能看。4.0 在工作台里给它们一个家。
...
- 目标：工作台内一个**文档中心**视图：左侧菜单（默认按群模板聚合，可切按时间排序），右侧渲染 Markdown；**群公告可编辑**；从会话/流程轨一键跳入。
- 边界：只聚合 `data/journals/` 的会话文档与熔炉相关 MD；`docs/`（仓库文档）不进这个视图。

## 2. 实施原则

1. **不另起第二个 web 服务**：并入现有 Express + Vue 应用（新增 `/docs` 视图 + `/api/docs` 路由）。第二个端口 = 第二套本机令牌/Origin 安全面 + 第二份打包负担；复用现成的 `requireLocalAccess`、UI 壳与三平台打包流水线。
...
- 流程轨/会话详情现有的「打开 MD」（起 OS 查看器）改为**默认跳文档中心内嵌视图**（`/docs?session=…&file=…`），保留「系统打开」为次按钮。
- WebSocket 事件复用既有 `announcement.updated`：文档中心开着时右侧内容自动刷新。
...
- [ ] `router.js` 加 `/docs`；`App.vue` 顶栏加「文档」入口
- [ ] `views/DocsHub.vue`：左菜单（双排序切换、群模板分组树）+ 右内容（渲染/编辑切换）
- [ ] `views/workbench/composables/useDocsHub.js`：列表状态、当前文档、保存与离开守卫
- [ ] `FlowRail`「打开 MD」改为内嵌跳转（保留系统打开次按钮）
...
- `/docs` 视图可用：默认群模板排序 + 可切时间排序；公告可读可编辑可保存。
