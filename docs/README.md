...
| 文档 | 用途 |
|------|------|
| [mvp.md](./mvp.md) | 宗旨、验收；`1.x` 已封 · `2.6.0` 已封 · `3.8.0` 已封（3.x）· 当前发布线 `4.9.0`（双引擎 TUI + 文档中心合并 + 桌面 Logo） |
| [ai-case-blog.md](./ai-case-blog.md) | **对外案例博客**（单一事实源）：标题/简介/分类/标签/封面/场景/痛点/AI 工具/用户/覆盖范围/频率/层级/提效/规划；README 对外叙事对齐本文 |
...
| [4.7.0-plan.md](./4.7.0-plan.md) | **4.7～5.x 总纲**（早期演进大纲；4.8/4.9 细节见下两份） |
| [4.8.0-plan.md](./4.8.0-plan.md) | **4.8.0 双引擎 TUI**：Cursor CLI + 熔炉、默认 TUI、桌面全屏；含落地跟踪 |
| [4.9.0-plan.md](./4.9.0-plan.md) | **4.9.0 文档中心与桌面图标**：Logo 托盘/exe、炼化 Skill、群报告并入文档中心；含落地跟踪 |
...
1. **README** = 对外/对内第一入口（安装、运行、功能现状、API 摘要）。对外叙事（痛点表、面向谁、AI 层级）对齐 [ai-case-blog.md](./ai-case-blog.md)，改定位先改案例文再同步 README。  
...
| 3.8.x | 已实施：重构与加固——engine/Workbench 拆分、复制逻辑单一来源、ESLint 进 CI、REST 收紧 `?token=`、web 纯逻辑测试。详见 [refactor-3.8-plan.md](./refactor-3.8-plan.md)。 |
| 4.x | 文档中心 4.0～4.2 已封；炼化/搜索 4.5～4.6；桌面窗口 4.7；双引擎 TUI 4.8；图标与文档中心合并 4.9。跟踪见 [4.8.0-plan.md](./4.8.0-plan.md)、[4.9.0-plan.md](./4.9.0-plan.md)。 |
