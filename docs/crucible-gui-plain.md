...
菜单、快捷键、模型条、登录态 **只在 TUI**。TUI 铺满原生终端，靠 xterm 滚轮上翻（吞备用屏，把对话留在主缓冲）。Grok 画面里那些可点按钮可能点不准，**以键盘快捷键为准**。更早的干净正文也可切回 GUI 气泡查看。
...
- 切 TUI：终端铺满中栏，滚轮上翻历史；画面里的可点按钮可能点不准，用键盘（Shift+Tab / Ctrl+x 等）。干净对话也可切 GUI 气泡。
- 附件路径仍是一行写入同一进程。
