...
| **Archive（归档）** | 释放资源；会话可解档续聊 / 节点重开；**空白新任务请另开聊** |
...
| **归档后随手输入** | 解档并记入**同一** Session；续跑流程用右侧「克隆并从此开始」 |
...
> **节点是死的，人是活的。** 场外是线性可扩展段落。回主线/重开：右侧正常节点「克隆并从此开始」（场外无此按钮）。**归档只释放资源**，会话可解档续跑。历史世代在右侧可折叠。
...
- 已归档：输入框提示「只释放资源，发送将解档」；续跑用右侧「克隆并从此开始」 
...
| ADR-017 | **完成/停止均归档**；归档 = 释放资源；发送/解档可续本会话；空白新任务另开聊 | 资源边界清晰，会话可流动重开 |
...
3. 成员/群可 CRUD、可无限克隆  
4. 完成/停止 → 归档（释放资源）；可解档 / 节点克隆重开；空白新任务另开聊  
5. 重启 = 克隆群 + 指定进程维护文档  
