...
| 项 | 内容 |
|----|------|
| **宗旨** | 让人与多智能体（含脚本）在 **同一群聊工作台** 协作：看见流程、关键点人机闸门、任务可归档可续，而不是能力散落在各终端 |
...
| **Archive（归档）** | 任务结束态；归档后该会话不再接收新输入；**用户再发消息 = 新任务** |
...
**诉求**：一次任务要有清晰边界——**做完或停掉都要归档**；之后可 **重启任务**（简单版，见 §2.0.1.1），或随手输入开一条全新任务。
...
> 「暂停」≠ 停止：暂停可继续同一任务；**停止/完成 = 归档**，不能再往该 Session 写业务消息。
...
创建任务 (active)
    → 运行 / 闸门 / 暂停（仍属同一任务）…
    → 被动完成  ──默认──► 自动归档 (archived)
    → 主动停止  ────────► 手动归档 (archived)
    → 被动停止（失败等）─► 归档 (archived)

archived 后可选：
    A. 随手发消息     → 同槽位 **全新空任务**（不绑旧台账，最简单）
    B. 点「重启任务」 → **必须** 克隆群聊 + 指定进程守护文档 → 新任务（见下）
```
...
| 规则 | 说明 |
|------|------|
| **完成默认归档** | 正常结束 **必须** 自动归档（toast：「已完成并归档」） |
| **停止即归档** | 主动/被动停止后同样归档，历史只读 |
| **归档后随手输入** | 发送内容 → 新建空白 Session（同原群模板槽位），**不**自动带旧台账 |
| **历史只读** | 消息/流程图/产物可回看，不可再闸门 |
| **列表** | 默认进行中；归档进「已归档」 |
| **台账联动** | 归档时 MD 状态 → done/archived/stopped |
...
| **`offsite`（场外协助）** | **流程外操作统归**（如 `@` 成员）；不参与线性推进 | 群里喊人帮忙，不改主流程指针 |
| `gate` | 强制审核 | 管理员/你过审 |
| `condition` | 按结果走边 | 群公告分支 |
| `parallel` | 多成员并行 | v1 可后置 |
| `tool` | 无 LLM 脚本成员 | 「机器人」静默执行也可冒泡日志 |

> **回主流程**：点额外节点「继续主流程」，或在右侧选正常节点「从此重新开始」。  
> **展示**：场外协助是主流程上的 **额外节点**，可插在任意步骤位置（按 step 顺序内联），琥珀色高亮，与正常「当前」(蓝) /「未执行」(红) 区分。  
> **示例**：上班上累了 → 流程中间插「点外卖」额外节点 → @成员办事 → 继续主流程。
...
- 已归档：输入框提示「发送将开始空白新任务」；**重启任务** 走克隆群 + 选台账向导
...
| **（归档后）发送消息** | 开 **空白** 新任务 | 不强制克隆群、不绑旧台账 |
| **重启任务** | 接着干（简单版） | **必须** 克隆群聊 + 指定进程维护文档；见 §2.0.1.1 |
...
|------|--------|------|
| 显示名 | 可配置 | |
| **手机** | **17312678391** | 交流用；复制 / `tel:` |
| **微信** | **17312678391**（默认同手机） | 技术交流主渠道 |
| 邮箱 / 主页 | 可选 | |
...
{ type: 'user.message', payload: { text, mentionAgentId? } }  // 已归档时由服务端开新任务
...
- [x] Session：线性引擎、闸门、自动/手动归档、归档后新任务  
...
| ADR-017 | **完成/停止均归档**；归档后随手输入=空白新任务 | 任务边界清晰 |
| ADR-018 | **重启任务**须克隆群聊 + 指定进程维护文档；v1 从流程起点再跑 | 续作靠台账承接；避免半残状态机；先做简单 |
...
| 作者微信号是否独立于手机 | 默认同号 17312678391 | 可在 author 配置中改 |
