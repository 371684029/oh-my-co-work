...
很多自动化仍然散落在 BAT、PowerShell、CLI、TUI 和不同 Agent 里：执行在终端，决策在聊天，进度靠人脑记，出错后很难复盘。
...
对外案例口径（标题、标签、层级、提效、规划）见 [案例博客](./docs/ai-case-blog.md)。
...
复现命令进终端卡 → 需要改代码时点桌宠开熔炉（Grok 或 Cursor CLI）→ 人确认补丁 → 回归脚本 → 提交。熔炉和脚本同一条群聊里插队，不用另开窗口记进度。外部 Agent 可复制成员页「炼化 Skill」指令，把 diff 与结论写回文档中心。
...
<p align="center"><sub>工作台右侧「文档中心」：群报告与台账同一栏，窄宽度横向排版、可折行</sub></p>

- 左栏按群模板（或时间）分类，中栏会话文件树，右栏渲染 Markdown。
- 工作台第三栏把原来的「群报告」并进「文档中心」，不再单独占一个 Tab。
- 只有 `ANNOUNCEMENT.md` 可编辑；节点台账只读，避免改坏溯源。
- 第三方 Agent 可用 `POST /api/docs/refine`（或成员页复制的 `agent-refine-skill` 指令）把输入 / diff / 结论追加进群报告。
...
| 群报告 | 自动汇总参数、节点输入输出和人工备注，落地 Markdown；工作台并入文档中心 |
| 文档中心 | 独立顶栏解耦只留 Logo；工作台第三栏内嵌文档中心（群报告 + 台账）；独立页面升级三栏分类布局；公告可编辑、全文搜索与按群导出 zip |
| 炼化 | 成员/步骤勾选后**保存当下**改源脚本（`ACW-REFINE`：ACW 输入环境变量 + stdout 标题），规格写入 `config.refine.format`；群聊跑完再整理进文档中心 |
| 更新与备份 | 启动检查更新默认开（设置可关，只读版本号、不自动安装）；设置页新增**一键备份与一键导出**（SQLite 完整性检查与本地快照、备份包安全校验下载、全量协同文档打包 ZIP）；关于页状态无缝联动 |
...
| 文档 | 内容 |
|------|------|
| [案例博客](./docs/ai-case-blog.md) | 对外案例单一事实源：痛点、场景、AI 层级、提效与规划（对齐 4.2.0） |
...
- [x] 4.5.0：熔炉炼化——成员/步骤炼化字段、执行后格式化进台账（stdout 优先）、规格回写不覆盖其它 config；**保存当下按契约改源文件**
