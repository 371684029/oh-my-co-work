...
| 熔炉 | 右侧桌宠；干活面 GUI/TUI 同一条 grok；**返回群聊只关皮**；顶栏「进程」里**关闭熔炉 / 新开熔炉**才清对话；短 AGENTS + rules 文件（不经命令行传 prompt）；GUI 附件写路径进同一进程；通栏满屏布局、深度思考/完成耗时胶囊卡片美化与 CLI 噪声深度过滤 |
...
| 文档中心 | 独立顶栏解耦只留 Logo；工作台第三栏内嵌文档中心 Tab（实时台账预览与导出）；独立页面升级三栏分类布局（群模板/时间跨度 -> 会话文件 -> 正文区）；公告可编辑、全文搜索与按群导出 zip |
| 更新与备份 | 启动检查更新默认开（设置可关，只读版本号、不自动安装）；**备份与从备份恢复**全面改造：支持通过文件浏览器选择目标/源文件或文件夹进行备份与还原（支持 `.tar.gz` 压缩包、`.sqlite` 库文件及目录）；关于页/设置页状态无缝联动 |
...
- [x] 4.2.1：文档中心解耦与工作台三栏集成（内嵌台账预览与群导出）、备份与从备份恢复支持自由选择文件与文件夹、熔炉通栏满屏与元信息胶囊卡片美化（深度思考与耗时结构化呈现）、下载安全与竞态加固
