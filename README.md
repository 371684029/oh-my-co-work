...
- 默认 **GUI**：去颜色的 TUI 尾部 + 底部输入。附件落到熔炉 `inbox/`，发送时把相对路径写成一行写进同一进程（不是 Grok 原生传文件）。只留一张大卡通头像。
- 开熔炉会写入 `data/furnace/AGENTS.md` 标记块，并用 `--prompt` 点名主持 / 适配 / 审核，让 Grok 按当前角色融入本场群。**不改** 你的全局 `~/.grok/AGENTS.md`。
