...
| 会话有边界 | 完成或停止 → 归档释放资源；可解档 / 节点重开；空白新任务另开聊 |
...
| 归档 | 释放进程与目录占用；**仍可解档 / 节点重开** |
| 新任务 | 左栏另开聊；归档后发送 = **解档续本会话**（不再自动空白新任务） |
| 聊天 | **名称可编辑**、**可删除** |
...
5. 归档后再发一条消息 → **解档并记入本会话**；续跑点右侧「克隆并从此开始」  
...
| POST | `/sessions/:id/unarchive` | 解档 |
| POST | `/sessions/:id/messages` | 发消息（已归档则先解档） |
