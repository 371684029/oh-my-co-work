...
> 最后更新: 2026-08-19 · 适用于 oh-my-co-work v3.4.0
...
版本现状：**3.4.0**。点开熔炉时把本轮合同写入 Grok 专用 cwd 的 `AGENTS.md` 标记块，并用短 `--prompt` 点名角色。熔炉是右侧桌宠：已装且已登录直接开 Grok TUI（默认满屏），否则弹教程。群聊同意最后一步即完成并归档；成员单聊不自动归档。服务端已上随机访问令牌与 Origin 校验；前端已支持终端卡、中栏 TUI 工作区、满屏与全屏。首页欢迎区居中，口号同一字号：人机协同 · 万物归元 · 皆可 Workflow · 终端守护者 · 熔炉连接一切。桌面壳与更多 CLI Adapter 仍后置。
