...
> 最后更新: 2026-08-17 · 适用于 oh-my-co-work v2.4.0
...
版本现状：**2.4.0**。服务端已上随机访问令牌与 Origin 校验（bootstrap 必须可信 Origin）；前端已支持终端卡、中栏 TUI 工作区、全屏、终端偏好、配额与日志脱敏；内嵌终端默认常驻；占用进程在设置里选择释放。JSONL Adapter 可将子工具提问转成对话闸门；桌面壳与更多 CLI Adapter 仍后置。
