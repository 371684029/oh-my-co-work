...
> 最后更新: 2026-08-18 · 适用于 oh-my-co-work v3.3.0 最终封板
...
版本现状：**3.3.0 最终封板**。熔炉（精灵入口、是否适配、工作流 prompt、Grok Build 教程）已落地。服务端已上随机访问令牌与 Origin 校验（本机同源即可领取令牌）；前端已支持终端卡、中栏 TUI 工作区、满屏与全屏、终端偏好、配额与日志脱敏；内嵌终端默认常驻；占用进程在设置里选择释放。JSONL Adapter 可将子工具提问转成对话闸门。首页主口号「皆可 Workflow」后加重标识「终端守护者」。桌面壳与更多 CLI Adapter 仍后置。
