...
版本现状：**3.7.0（3.x 最终封板）**。点精灵开熔炉默认铺满 GUI（欢迎卡 / 当前屏可读正文，可缩小回三栏、可切 TUI）；桌宠循环 GIF（可戳）；开炉短 AGENTS + `--prompt`。GUI 附件把路径写进同一 PTY。未装或未登录弹教程。没有自研 Grok HTTP 客户端。群聊同意最后一步即完成并归档；成员单聊不自动归档。服务端已上随机访问令牌与 Origin 校验。首页欢迎区居中，口号含终端守护者 · 熔炉连接一切。托盘独立窗与更多 CLI Adapter 仍后置。
