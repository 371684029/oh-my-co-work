...
> 最后更新: 2026-08-17 · 适用于 oh-my-co-work v2.2.2
...
版本现状：**2.2.2**。服务端已上随机访问令牌与 Origin 校验（bootstrap 必须可信 Origin）；前端已支持终端卡、中栏 TUI 工作区、全屏与会话治理；内嵌终端默认常驻（启动即推进、不判超时）；占用进程在设置里选择释放。
