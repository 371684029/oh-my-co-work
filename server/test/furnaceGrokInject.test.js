...
test('inject writes AGENTS block, never global grok home', () => {
...
test('inject into furnace home does not clobber ACTIVE.md', () => {
  const pack = activateFurnaceRole(FURNACE_ROLE.SESSION, {
    situation: { intent: '主持这一场' },
  })
  const original = fs.readFileSync(pack.activeMd, 'utf8')
  const r = maybeInjectGrokFurnace(pack, {
    grok: { writeRules: true, command: 'grok' },
    cwd: furnaceHomeDir(),
  })
  assert.equal(r.wrote, true)
  assert.equal(fs.readFileSync(pack.activeMd, 'utf8'), original)
  const agents = fs.readFileSync(path.join(furnaceHomeDir(), 'AGENTS.md'), 'utf8')
  assert.ok(agents.includes(FURNACE_AGENTS_BEGIN))
  assert.ok(agents.includes('主持这一场'))
})
...
test('ACW_GROK_WORKSPACE wins over settings path', () => {
  assert.equal(
    resolveGrokWorkspaceDir({ workspaceDir: '/tmp/nope' }),
    path.resolve(process.env.ACW_GROK_WORKSPACE),
  )
})

test('default workspace is furnace home when env unset', () => {
